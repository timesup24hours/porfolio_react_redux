...
import * as addProductFormActions
  from '../../store/actions/addProductFormActions'
import {
  addProductionFormValidation,
  submitValidation,
  initialValidate
} from './addProductionFormValidation'
...
class AddProductForm extends Component {
  ...
  getFormNameAndValue = (listDesc, images) => ({
    [document.querySelector('#name').name]: document.querySelector('#name')
      .value,
    [document.querySelector('#category').name]: document.querySelector(
      '#category'
    ).value,
    [document.querySelector('#subCategory').name]: document.querySelector(
      '#subCategory'
    ).value,
    [document.querySelector('#stock').name]: document.querySelector('#stock')
      .value,
    [document.querySelector('#numberOfStock').name]: document.querySelector(
      '#numberOfStock'
    ).value,
    [document.querySelector('#desc').name]: document.querySelector('#desc')
      .value,
    [document.querySelector('#name').name]: document.querySelector('#name')
      .value,
    [document.querySelector('#brand').name]: document.querySelector('#brand')
      .value,
    [document.querySelector('#salePrice').name]: document.querySelector(
      '#salePrice'
    ).value,
    listDesc: listDesc || this.state.listDesc,
    [document.querySelector('#onSale').name]: document.querySelector('#onSale')
      .value,
    [document.querySelector('#size').name]: document.querySelector('#size')
      .value,
    [document.querySelector('#images').name]: images || this.state.images,
    [document.querySelector('#soldBy').name]: document.querySelector('#soldBy')
      .value
  })
  ...
  validatingFormValueDisableUploadButton = (listDesc, images) => {
    const { isValid } = submitValidation(
      this.getFormNameAndValue(listDesc, images)
    )
    if (!isValid) {
  ...
  handleListDescError = (e, index) => {
    if (e.target.id === 'listDesc') {
      if (
        this.state.errors.listDesc === 'At least 2 Brief Description' &&
        this.state.listDesc.length >= 2
      ) {
      ...
      if (
        this.state.errors.listDesc === 'required' &&
        this.state.listDesc.length !== 0
      ) {
        let copy = Object.assign({}, this.state.errors)
        delete copy.listDesc
        this.setState({ errors: copy })
  ...
  handleUploadProduct = e => {
    ...
    document.querySelector('#desc').parentElement.firstChild.textContent =
      'Long Description'
  ...
  render() {
    ...
    let filesPreviewDiv = this.state.images.map((f, i) => {
      return (
        <div className="AddProductForm-image-each" key={i}>
          ...
          <img
            ...
            alt=""
          />
          <div className="AddProductForm-image-text flow-text">
            {f.name} ({f.type}) - size: {bytesToSize(f.size)}
          </div>
        </div>
      )
    })
            ...
            <div className="col-sm-11 col-xs-11">
              <div className="form-group">
                ...
                <div className="addProductForm-listDesc-cancel">
                  <i
                    className="material-icons hover-cursor-pointer"
                    onClick={e => this.handelListDescRemove(e, i)}
                  >
                    cancel
                  </i>
                </div>
                <span className="text-danger addProductForm-invalid-span">
                  {this.state.errors[`listDesc${i}`]}
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="AddProductForm-container container">
        ...
        <div className="row">
          <div className="col-sm-12">
            <div className="form-group">
              <label className="" htmlFor="desc">Long Description</label>
              <textarea
          ...
          <div className="col-sm-12">
            <div className="form-group">
              ...
              <button
                ...
                onClick={this.handleAddBriefDescription}
              >
                Add Description
              </button>
              <span className="text-danger addProductForm-invalid-span">
                {this.state.errors.listDesc}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="row">
          ...
          <div className="col-sm-4">
            <div className="form-group">
              <label className="active" htmlFor="numberOfStock">
                Number of Stock
              </label>
              <input
                className="form-control"
                ...
                onKeyPress={e => {
                  if (
                    e.charCode === 101 || e.charCode === 45 || e.charCode === 46
                  ) {
                    e.preventDefault()
                  }
                }}
              ...
              />
              <span className="text-danger addProductForm-invalid-span">
                {this.state.errors.numberOfStock}
              </span>
            </div>
          </div>
        ...
        <div className="row">
          <div className="col-sm-12">
            <div className="form-group">
              <label htmlFor="images" className="btn btn-default">
                Upload images
              </label>
              <input
                style={{ display: 'none' }}
...
const mapDispatchToProps = dispatch => ({
  addProductRequest: payload =>
    dispatch(productActions.addProductRequest(payload)),
  setDataToProductDataFormReducer: payload =>
    dispatch(addProductFormActions.setDataToProductDataFormReducer(payload))
})
